...
s_thresh, sx_thresh, dir_thresh, m_thresh, r_thresh = (120, 255), (20, 100), (0.5, 1.5), (30, 100), (200, 255)

# load the calibration
calib_file = r'C:\Users\Mohtashim Butt\Documents\Fall-2022\CV\Project\driving-lane-departure-warning\camera_cal\calibration_pickle.p'
...
def gradient_pipeline(image, ksize = 2, sx_thresh=(20, 100), sy_thresh=(20, 100), m_thresh=(30, 100), dir_thresh=(0.7, 1.3)):
...
def find_edges(img, s_thresh=s_thresh, sx_thresh=sx_thresh, dir_thresh=dir_thresh):
    ...
    # cv2.imshow("HUI hls:",hls)
    # cv2.waitKey(0)
    s_channel = hls[:,:,2]
    # cv2.imshow("hahahahah:",s_channel)
    # cv2.waitKey(0)
...
def full_search(binary_warped, visualization=False):
    # print("Slice int:",binary_warped.shape[0]/2)
    ...
    for window in range(nwindows):
        ...
        if visualization:
            ...
            cv2.rectangle(out_img,(int(win_xright_low),int(win_y_low)),(int(win_xright_high),int(win_y_high)),(255,0,0), 2)
        # cv2.imshow("HUIHUIHUI",out_img)
        # cv2.waitKey(0)
        ...
        # print("F:",(nonzerox < win_xright_high))
    ...
    # print("F:",left_lane_inds)
    lefty = nonzeroy[left_lane_inds]
    # print("nonzerox[right_lane_inds]:",right_lane_inds,nonzerox)
    # print("nonzeroy[right_lane_inds]:",right_lane_inds,nonzeroy)
...
def measure_lane_curvature(ploty, leftx, rightx, visualization=False):
    ...
    ym_per_pix = 10/(frame_height/input_scale) # meters per pixel in y dimension
    xm_per_pix = LANEWIDTH/(frame_width/input_scale) # meters per pixel in x dimension
    ...
    if leftx[0] - leftx[-1] > 70/input_scale:
        curve_direction = 'Left curve'
    elif leftx[-1] - leftx[0] > 70/input_scale:
...
def compute_car_offcenter(ploty, left_fitx, right_fitx, undist):
    ...
    # print("yelo",offcenter)

    return offcenter, pts


def create_output_frame(offcenter, pts, undist_ori, fps, curvature, curve_direction, binary_sub, threshold=0.75):
    """
...
def process_frame(img, visualization=False):
    ...
    # cv2.imshow("binary image:",binary_warped)
    # cv2.waitKey(0)
