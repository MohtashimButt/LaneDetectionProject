...
class LaneNetPostProcessor(object):
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        fucking_list=[] #temporary list made by me
        for lane_index, coords in enumerate(lane_coords):
            ...
            for index in range(0, plot_y.shape[0], 5):
                ...
                fucking_list.append([src_x, src_y])

            src_lane_pts.append(lane_pts)
        
        print("src_lane_pts",src_lane_pts)
        ...
        # print("single_lane_pt_x:",single_lane_pt_x)
        # print("single_lane_pt_y:",single_lane_pt_y)
        # for i in range(0,len(fucking_list)):
        #     for j in range(0,2):
        #         fucking_list[i][j]=int(fucking_list[i][j])

        # for i in range(0,len(single_lane_pt_y)):
        #     single_lane_pt_y[i]=float(single_lane_pt_y[i])
        x_vals = np.array(fucking_list, dtype=np.float32)[:,0]
        y_vals = np.array(fucking_list, dtype=np.float32)[:,1]

        # with open("sample.json", "w") as outfile:
        #     json.dump({'x_vals':x_vals.tolist(), 'y_vals':y_vals.tolist()}, outfile)

        #dumping src_lane wali list:
        for i in range(0,len(src_lane_pts)):
            for j in range(0,len(src_lane_pts[i])):
                for k in range(0,2):
                    src_lane_pts[i][j][k]=int(src_lane_pts[i][j][k])
        with open(r"C:\Users\Mohtashim Butt\Documents\Fall-2022\CV\Project\curved_path_ki_jasons\sample_9.json", "w") as outfile:
            json.dump({'lanes':src_lane_pts}, outfile)
